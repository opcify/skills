...
This library provides a simple Python interface to DuckDuckGo's search API
without requiring API keys or authentication.

**API note (breaking change in ddgs 9.x).** Earlier versions of `ddgs`/`duckduckgo_search`
used `keywords=` as the first argument to `DDGS.text()` / `news()` / `images()` /
`videos()`. From ddgs 9.x onward the parameter is named **`query=`**. The
`scripts/search.py` in this skill already uses the new signature; if you see
an error like `DDGS.text() missing 1 required positional argument: 'query'`,
you are running against an older checkout that still passes `keywords=` —
upgrade the skill or reinstall `ddgs`.
...
- **"Missing required dependency"**: Run `pip install "ddgs>=9"`
- **`DDGS.text() missing 1 required positional argument: 'query'`**: You are
  running against a pre-9.x ddgs API. Either upgrade `ddgs` (`pip install -U
  "ddgs>=9"`) or ensure the script is passing `query=` (not `keywords=`).
  This skill's `scripts/search.py` already uses `query=`.
...
- **Unexpected Runtime Exec Error**: The DuckDuckGo API may have changed —
  check the upstream `ddgs` project (<https://github.com/deedy5/ddgs>) for
  the current method signatures and update `scripts/search.py` accordingly.
