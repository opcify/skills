...
Search the web using DuckDuckGo via the `ddgs` Python package. Supports web,
news, image, and video search with multiple output formats.

API note: since ddgs 9.x the first positional argument for every search
method is `query=` (previously `keywords=`). If you see
"DDGS.text() missing 1 required positional argument: 'query'", an older
branch is calling keywords= — it has been corrected in this script.
