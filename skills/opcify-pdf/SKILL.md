...
description: >
  ...
  MD2PDF (quick Markdown to PDF): "convert Markdown to PDF", "markdown to pdf",
  "quick PDF from .md", "render this markdown".
  This skill uses a token-based design system for CREATE/REFORMAT routes, and
  markdown-convert for quick faithful Markdown→PDF conversion (MD2PDF route).
  Prefer CREATE/REFORMAT when appearance matters; use MD2PDF for quick, clean output.
license: MIT
metadata:
  version: "1.0"
...
This skill depends on Python and Node packages that are **not** preinstalled in the gateway image. The easiest way to install everything is to let `make.sh` do it:

```bash
bash scripts/make.sh fix
```

That command installs into the gateway's openclaw-env venv (Python) and into the global npm prefix (Node) — both of which are bind-mounted, so the install persists across invocations and you only need to run `fix` once per workspace.

If you prefer to install manually:

```bash
# Python packages used by palette.py, cover.py, render_body.py, merge.py,
# fill_inspect.py, fill_write.py, reformat_parse.py:
uv pip install --python /home/node/.openclaw-env/bin/python3 reportlab pypdf matplotlib

# Node package used by render_cover.js to rasterize the cover HTML → PDF:
npm install -g playwright
```

For the **MD2PDF** route, also install the `markdown-convert` CLI:

```bash
uv pip install --python /home/node/.openclaw-env/bin/python3 markdown-convert
```
...
It should print ✓ for python3, reportlab, pypdf, node, playwright, and (optionally) matplotlib.
...
| Quick Markdown → PDF (preserves original formatting) | **MD2PDF** | `markdown-convert` CLI |
...
**Rule:** when the user wants a quick, faithful conversion of Markdown to PDF (no custom cover or design tokens), use **MD2PDF**. When they want styled/branded output, use **REFORMAT**.
...
Direct Markdown → PDF conversion using [markdown-convert](https://github.com/Julynx/markdown_convert). Preserves the original Markdown formatting faithfully — no custom cover page or design tokens. Best for quick, clean PDF output from Markdown files.

```bash
bash scripts/make.sh md2pdf --input doc.md --out output.pdf
```
...
| `--out FILE` | Output PDF path (defaults to same name with `.pdf` extension) |
| `--css FILE` | Custom CSS file for styling |
| `--extras LIST` | Comma-separated extras: `math`, `mermaid`, `syntax-highlighting`, `task-lists`, `table-of-contents`, `admonitions`, `anchors`, `custom-spans`, `highlights`, `vega-lite`, `inline-math`, `block-math`, `schemdraw`, `dynamic-tables`, `dynamic-queries`. Uses all extras if not specified. |
| `--security LEVEL` | `basic` (allow inline HTML + local files), `default` (no inline HTML), `strict` (no HTML, no JS, no internet) |

**When to use MD2PDF vs REFORMAT:**
- **MD2PDF** — quick, faithful Markdown rendering. Good for README files, notes, technical docs.
- **REFORMAT** — styled output with cover page, design tokens, accent colors. Good for client-facing reports.
...
| `reportlab` | `render_body.py` | `uv pip install reportlab` (pre-installed) |
| `pypdf` | fill, merge, reformat | `uv pip install pypdf` (pre-installed) |
| `markdown-convert` | md2pdf route | `uv pip install markdown-convert` (pre-installed) |
| Node.js 18+ | `render_cover.js` | system |
| `playwright` + Chromium | `render_cover.js` | `npm install -g playwright` (pre-installed) |
