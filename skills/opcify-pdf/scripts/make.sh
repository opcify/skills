...
#   md2pdf --input F --out FILE     Direct Markdown → PDF via markdown-convert
#         --css FILE               Optional custom CSS
#         --extras E1,E2,...        Optional extras (math, mermaid, etc.)
...
# Resolve Python to the openclaw-env venv when it exists (Docker runtime),
# otherwise fall back to whatever python3 is first on PATH. Using an absolute
# path removes ambiguity between the interpreter used for `python3 -c "import X"`
# checks and the interpreter `uv pip install --python "$PY"` installs into — a
# mismatch is the most common cause of "installed but still missing" reports.
if [[ -x /home/node/.openclaw-env/bin/python3 ]]; then
  PY="/home/node/.openclaw-env/bin/python3"
else
  PY="python3"
fi
NODE="node"
...
cmd_check() {
  ...
  if [[ -x "$PY" ]] || command -v "$PY" &>/dev/null; then
    green "  ✓ python3 $($PY --version 2>&1 | awk '{print $2}')"
  else
  ...
  # Playwright (Node package — render_cover.js uses `require('playwright')`)
  if node -e "require('playwright')" 2>/dev/null || \
    ...
    green "  ✓ playwright (node)"
  else
    yellow "  ⚠ playwright (node) not found  (run: make.sh fix)"
    ok=false
  fi
...
cmd_fix() {
  ...
  # Python packages — reportlab, pypdf (required) + matplotlib (optional).
  # `--python "$PY"` pins the install target to the same interpreter used by
  # the check commands, so there is no ambiguity about VIRTUAL_ENV or PATH.
  # stderr is NOT suppressed — real errors need to be visible.
  if command -v uv &>/dev/null; then
    if uv pip install --python "$PY" reportlab pypdf matplotlib; then
      green "  ✓ Python packages installed (reportlab, pypdf, matplotlib)"
    else
      red  "  ✗ uv pip install failed (see error output above)"
      rc=3
    fi
  else
    red  "  ✗ uv not found — cannot install Python packages automatically"
    rc=2
  fi

  # Node playwright — render_cover.js uses require('playwright'). Install it
  # globally into the npm-global prefix if missing. Chromium itself is already
  # present at PLAYWRIGHT_BROWSERS_PATH from the container image.
  if ! node -e "require('playwright')" 2>/dev/null && \
     ! node -e "require(require('child_process').execSync('npm root -g').toString().trim()+'/playwright')" 2>/dev/null; then
    if command -v npm &>/dev/null; then
      if npm install -g playwright; then
        green "  ✓ Node playwright installed"
      else
        red  "  ✗ npm install -g playwright failed (see error output above)"
        rc=3
      fi
    else
      red  "  ✗ npm not found — cannot install Node playwright automatically"
      rc=2
    fi
  fi
  ...
  if [[ $rc -eq 0 ]]; then
    ...
    yellow "\nSome installs failed — fix the errors above and re-run: bash make.sh fix"
  fi
  exit $rc
}
...
# ── md2pdf ────────────────────────────────────────────────────────────────────
cmd_md2pdf() {
  local input="" out="" css="" extras="" security=""
  ...
  if [[ -z "$input" ]]; then
    echo "Usage: make.sh md2pdf --input doc.md [--out output.pdf] [--css style.css] [--extras math,mermaid] [--security default]"
    exit 1
  fi
  ...
  # Check markdown-convert is available
  if ! command -v markdown-convert &>/dev/null; then
    red "  ✗ markdown-convert not found"
    yellow "  Install: uv pip install markdown-convert"
    exit 2
  fi

  bold "Converting: $input → $out"

  local mc_args=("$input" "--out=$out")
  [[ -n "$css" ]]      && mc_args+=("--css=$css")
  [[ -n "$extras" ]]   && mc_args+=("--extras=$extras")
  [[ -n "$security" ]] && mc_args+=("--security=$security")

  markdown-convert "${mc_args[@]}"
  green "  ✓ PDF created: $out"
}
...
main() {
  if [[ $# -lt 1 ]]; then
    ...
    echo "  md2pdf --input doc.md             MD2PDF: direct Markdown → PDF via markdown-convert"
    echo "         [--out output.pdf] [--css style.css]"
    echo "         [--extras math,mermaid,...] [--security basic|default|strict]"
