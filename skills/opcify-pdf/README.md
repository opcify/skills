...
| `playwright` + Chromium | `render_cover.js` | `npm install -g playwright` (pre-installed) | `npx playwright install chromium` (pre-installed) |
...
| `reportlab` | Body page rendering | `uv pip install reportlab` (pre-installed) |
| `pypdf` | Merging PDFs | `uv pip install pypdf` (pre-installed) |
| `markdown-convert` | Direct Markdown → PDF | `uv pip install markdown-convert` (pre-installed) |
| Node.js 18+ | Cover rendering | system |
| `playwright` | Headless Chromium for cover | `uv pip install playwright` (pre-installed) |
