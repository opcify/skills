...
This skill depends on Python packages that are **not** preinstalled in the gateway image. Before invoking `xlsx_reader.py`, `xlsx_unpack.py`, `xlsx_pack.py`, or any other script in this skill for the first time, install them into the gateway's venv:

```bash
uv pip install pandas openpyxl
```

Run this once per workspace — the venv is bind-mounted, so subsequent script invocations (and subsequent tasks) reuse it.
